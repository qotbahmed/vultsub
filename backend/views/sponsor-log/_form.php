...
<div class="sponsor-log-form">
    ...
    <div class="d-flex align-items-center flex-wrap section_header justify-content-between gap-3">
        ...
        <div class="mb-0 d-inline-flex align-items-center gap-2">
            <?= Html::submitButton(
                $model->isNewRecord ?
                    '<span class="isax isax-add"></span> ' . Yii::t('backend', 'Create') :
                    '<span class="isax isax-edit"></span> ' . Yii::t('backend', 'Update'),
            ...
            <div class="row">
                ...
                <div class="col-md-4">
                    <?= $form->field($model, 'sponsor_id')->widget(\kartik\widgets\Select2::classname(), [
                        'data' => \yii\helpers\ArrayHelper::map(
                            \common\models\Sponsors::find()->orderBy('id')->asArray()->all(),
                            'id', 'title'
                        ),
                        'options' => ['placeholder' => Yii::t('backend', 'Choose Sponsors')],
                        'pluginOptions' => [
                            'allowClear' => true
                        ],
                    ]) ?>
                ...
                <div class="col-md-4">
                    <?= $form->field($model, 'amount')->textInput([
                        'maxlength' => true,
                        'placeholder' => Yii::t('backend', 'Amount')
                    ]) ?>
                </div>
            </div>
        </div>
