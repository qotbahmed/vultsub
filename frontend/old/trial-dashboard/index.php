...
<head>
    ...
    <title>لوحة تحكم التجربة - Vult</title>
    ...
    <div class="container my-5">
        ...
        <div class="trial-banner" id="trialBanner">
            <div class="loading">
                ...
                <p class="mt-3">جاري تحميل معلومات التجربة...</p>
            </div>
        </div>

        <!-- Trial Status Card -->
        <div class="trial-card" id="trialStatusCard" style="display: none;">
            <div class="row align-items-center">
                <div class="col-md-8">
                    <h4 class="fw-bold mb-3">حالة تجربتك المجانية</h4>
                    <div class="row">
                        <div class="col-md-6">
                            <p class="mb-2"><strong>اسم الأكاديمية:</strong> <span id="academyName">-</span></p>
                            <p class="mb-2"><strong>البريد الإلكتروني:</strong> <span id="userEmail">-</span></p>
                        </div>
                        <div class="col-md-6">
                            <p class="mb-2"><strong>حالة الطلب:</strong> <span id="requestStatus" class="badge">-</span></p>
                            <p class="mb-2"><strong>الأيام المتبقية:</strong> <span id="daysLeft" class="countdown">-</span></p>
                        </div>
                    </div>
                </div>
                <div class="col-md-4 text-end">
                    <button class="btn btn-trial btn-lg" onclick="upgradeNow()">
        ...
        <!-- Features Grid -->
        <div class="row" id="featuresGrid" style="display: none;">
            <div class="col-lg-4 col-md-6 mb-4">
                <div class="feature-card">
                    <i class="fas fa-users fa-3x text-primary mb-3"></i>
                    <h5 class="fw-bold">إدارة اللاعبين</h5>
                    <p class="text-muted">إضافة وإدارة بيانات اللاعبين</p>
                    <a href="../players-management/" class="btn btn-outline-primary">ابدأ الآن</a>
                </div>
            </div>
        ...
        <!-- Upgrade Section -->
        <div class="trial-card" id="upgradeSection" style="display: none;">
    ...
    <script>
        ...
        async function loadTrialStatus() {
            try {
                ...
                const email = urlParams.get('email') || localStorage.getItem('trial_email');
                ...
                const response = await fetch(`../api/trial-management.php?endpoint=trial-status&email=${encodeURIComponent(email)}`);
                ...
                if (data.success) {
                    ...
                    showError('حدث خطأ في تحميل بيانات التجربة: ' + data.error);
                }
            } catch (error) {
        ...
        function displayTrialStatus() {
            ...
            const statusCard = document.getElementById('trialStatusCard');
            ...
            document.getElementById('academyName').textContent = trialData.academy_name || 'غير محدد';
            document.getElementById('userEmail').textContent = localStorage.getItem('trial_email') || 'غير محدد';
            
            // Update request status
            const statusBadge = document.getElementById('requestStatus');
            const statusText = getStatusText(trialData.request_status);
            const statusClass = getStatusClass(trialData.request_status);
            statusBadge.textContent = statusText;
            statusBadge.className = `badge ${statusClass}`;
            
            // Update days left
            const daysLeft = document.getElementById('daysLeft');
            if (trialData.is_trial_active) {
                daysLeft.textContent = trialData.trial_days_left;
                daysLeft.style.color = trialData.trial_days_left <= 2 ? '#dc3545' : '#ff6b35';
            } else {
                daysLeft.textContent = 'انتهت';
                daysLeft.style.color = '#dc3545';
            }
            
            // Show appropriate sections
            if (trialData.is_trial_active) {
                featuresGrid.style.display = 'block';
                if (trialData.trial_days_left <= 3) {
        ...
        function upgradeNow() {
            window.location.href = '../?subdomain=pricing';
        }
        ...
        function showError(message) {
            ...
            banner.innerHTML = `
                <div class="text-center">
                    ...
                    <h5>حدث خطأ</h5>
                    <p>${message}</p>
                    <a href="../?subdomain=signup" class="btn btn-light">تسجيل جديد</a>
                </div>
            `;
        }
    </script>
