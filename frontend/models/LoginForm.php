...
class LoginForm extends Model
{
    ...
    {
        return [
            'username' => Yii::t('backend', 'Identity'),
            'identity' => Yii::t('backend', 'Mobile/Email'),
    ...
    public function validatePassword()
    {
        if (!$this->hasErrors()) {
            ...
            if (!$user || !$user->validatePassword($this->password)) {
                $this->addError('password', Yii::t('backend', 'Incorrect identity or password.'));
            }
        }
    }
    ...
    public function login()
    {
        ...
        if (Yii::$app->user->login($this->getUser(), $duration)) {
            if (!Yii::$app->user->getIdentity()->myRole(User::ROLE_USER)) {
                Yii::$app->user->logout();
